...
import { newId }           from '@/lib/patientDb'
import type { ParseDiagnostics } from '@/lib/excelParser'
import { DiagnosticPanel } from './DiagnosticPanel'

export function DropZone() {
  const appendFromUpload = useFilterStore(s => s.appendFromUpload)

  const [parsing,      setParsing]      = useState(false)
  const [parseError,   setParseError]   = useState<string | null>(null)
  const [lastDiag,     setLastDiag]     = useState<{ diag: ParseDiagnostics; filename: string } | null>(null)
  ...
  const onDrop = useCallback(async (accepted: File[]) => {
    ...
    try {
      ...
      if (result.patients.length === 0) throw new Error('No valid patient rows found — check the diagnostic panel')

      const uploadId = newId()
      const { added, updated } = await appendFromUpload(result.patients, {
        id:          uploadId,
        filename:    file.name,
        timestamp:   Date.now(),
        sheetName:   result.diagnostics.selectedSheet,
        headerRow:   result.diagnostics.headerRow,
        rowCount:    result.patients.length,
        status:      result.diagnostics.failedRows > 0 ? 'partial' : 'ok',
        diagnostics: result.diagnostics,
      })

      setLastDiag({ diag: result.diagnostics, filename: file.name })

      const total = useFilterStore.getState().dataset.length
      if (updated > 0) {
        showToast(`Loaded ${added} new + ${updated} updated patients. Total in library: ${total.toLocaleString()}.`)
      } else {
        showToast(`Loaded ${added} new patients. Total in library: ${total.toLocaleString()}.`)
      }
    } catch (err) {
    ...
    <div>
      ...
      >
        ...
        <h2 className="relative z-10 font-serif text-[32px] font-normal tracking-[-0.8px] leading-[1.05] text-ink-900 mb-3">
          {parsing ? 'Parsing…' : <>Add to <em className="gradient-text not-italic">your library</em></>}
        </h2>

        <p className="relative z-10 text-ink-500 text-[14px] leading-[1.55] max-w-[540px] mx-auto">
          Each file is appended to your growing patient library. Columns are auto-mapped — no formatting required.
        </p>
      ...
      {/* Diagnostic panel */}
      {lastDiag && (
        <DiagnosticPanel diagnostics={lastDiag.diag} filename={lastDiag.filename} />
      )}

      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 bg-ink-900 text-white text-[13px] font-medium px-5 py-3 rounded-xl shadow-xl animate-fade-in max-w-sm">
          {toast}
        </div>
      )}
    </div>
